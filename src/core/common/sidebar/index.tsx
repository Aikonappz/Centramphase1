...
const Sidebar = () => {
  ...
  const filteredSidebar = (SidebarDataTest as any[])
    .filter((menu: any) =>
      !menu.roles || menu.roles.includes(loginRole || "")
    )
    .map((menu: any) => ({
      ...menu,
      submenuItems: menu.submenuItems
        ?.filter((item: any) =>
          !item.roles || item.roles.includes(loginRole || "")
        )
        .map((item: any) => ({
          ...item,
          submenuItems: item.submenuItems?.filter((sub: any) =>
            !sub.roles || sub.roles.includes(loginRole || "")
          )
        }))
    }));
  return (
